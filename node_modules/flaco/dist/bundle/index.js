...
const updateDomNodeFactory = (method) => (items) => smartTableOperators.tap(domNode => {
...
const setAttributes = (attributePairs) => smartTableOperators.tap((dom) => {
...
const removeAttributes = (attributes) => smartTableOperators.tap((dom) => {
...
const lifeCycleFactory = (method) => smartTableOperators.curry((fn, comp) => (props, ...args) => {
    const n = comp(props, ...args);
    const applyFn = () => fn(n, ...args);
    const current = n[method];
    n[method] = current ? smartTableOperators.compose(current, applyFn) : applyFn; // allow multiple hooks;
    return n;
});
...
const updateEventListeners = (newNode, oldNode) => {
    ...
    return newNodeEvents.length || oldNodeEvents.length ?
        smartTableOperators.compose(removeEventListeners(oldNodeEvents), addEventListeners(newNodeEvents)) : noop;
};
const updateAttributes = (newVNode, oldVNode) => {
    ...
    return smartTableOperators.compose(removeAttributes(attributesToRemove), setAttributes(newNodeKeys.map(pairify(newVNodeProps))));
};
...
const mount = smartTableOperators.curry((comp, initProp, root) => {
...
const withState = (comp) => (initialState) => {
    ...
    return smartTableOperators.compose(onMount(setUpdateFunction), onUpdate(setUpdateFunction))(wrapperComp);
};
const withIdempotentState = (comp) => {
    ...
    return (initialProps) => {
        ...
        return smartTableOperators.compose(onMount(setUpdateFunction), onUpdate(setUpdateFunction), onUnMount(removeInstance))(wrapperComp);
    };
};
...
const escapeHTML = s => String(s)
    ...
    .replace(/</g, '&lt;')
    .replace(/>/g, '&gt;');
const renderAsString = smartTableOperators.curry((comp, initProp) => {
    const vnode = isVNode(comp) ? comp : comp(initProp || {});
